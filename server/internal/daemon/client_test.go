...
func TestFailTask_RetriesOnTransient5xxThenSucceeds(t *testing.T) {
	...
	c := NewClient(srv.URL)
	if err := c.FailTask(context.Background(), FailTaskParams{
		TaskID:                "task-1",
		ErrMsg:                "boom",
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			call: func(c *Client) error {
				return c.CompleteTask(context.Background(), CompleteTaskParams{
					TaskID:           "task-1",
					Output:           "done",
					WorkDir:          "/tmp/wd",
					RetiredSessionID: "POISONED-S",
				})
			},
		},
		{
			...
			call: func(c *Client) error {
				return c.FailTask(context.Background(), FailTaskParams{
					TaskID:           "task-1",
					ErrMsg:           "boom",
					WorkDir:          "/tmp/wd",
...
func TestTerminalReportsOmitEmptyRetiredSessionID(t *testing.T) {
	...
	if err := NewClient(srv.URL).CompleteTask(context.Background(), CompleteTaskParams{
		TaskID:    "task-1",
		Output:    "done",
		SessionID: "sess-1",
		WorkDir:   "/tmp/wd",
