...
func notifyIssueSubscribers(ctx context.Context, queries *db.Queries, bus *events.Bus, n issueSubscriberNotification) (map[string]bool, map[string]bool) {
	...
	statusIsHandoff, err := issueStatusIsHandoff(ctx, queries, parseUUID(n.WorkspaceID), n.IssueStatus)
	if err != nil {
		slog.Warn("resolve issue status handoff for delegated delivery; delivering",
			"issue_id", n.SubscriberIssueID, "status", n.IssueStatus, "error", err)
	...
	for _, sub := range subs {
		...
		if !deliverToSubscriber(sub.Reason, n.NotifType, statusIsHandoff) {
